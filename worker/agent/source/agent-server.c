...
static void
websocket_callback(gchar* operation,
				   gchar* data,
				   AgentServer* agent)
{
	if(!g_strcmp0(operation,"/cluster/Initialize")) {
		session_initialize(agent)? SOUP_STATUS_OK : SOUP_STATUS_BAD_REQUEST;
	} else if(!g_strcmp0(operation,"/cluster/Terminate")) {
		session_terminate(agent)? SOUP_STATUS_OK : SOUP_STATUS_BAD_REQUEST;
	} else if(!g_strcmp0(operation,"/cluster/Shell")) {
		initialize_shell_session_websocket(agent,data)? SOUP_STATUS_OK : SOUP_STATUS_BAD_REQUEST;
	}
}
